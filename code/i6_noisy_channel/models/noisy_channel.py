    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        for i, cm_input in enumerate(cm_input_ids):
            ...
            padding = torch.ones(difference, dtype=torch.int64).to(self.channel_model.device)
        ...
        cm_attention_mask[cm_input_ids == 1] = 0

        loss_fct = torch.nn.CrossEntropyLoss(reduction="none", ignore_index=-100)

        new_input_ids = sequence_outputs["sequences"][:,1:].clone()
    ...
    def beam_sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            next_token_scores = logits_processor(input_ids, next_token_scores)
            next_token_scores = next_token_scores + beam_scores[:, None].expand_as(next_token_scores)
            if hasattr(self.config, "generation_typical_p"):
                if self.config.generation_typical_p < 1.0:
                    logits_warper = TypicalLogitsWarper(mass=self.config.generation_typical_p, min_tokens_to_keep=(2 if num_beams > 1 else 1))
...
class OnlineNoisyChannelModelForConditionalGeneration(PretrainedNoisyChannelModel):
    ...
    def forward(
        ...
        **unused,
        ...
        dm_encoder_outputs, _, _ = tuple(encoder_outputs.last_hidden_states)
        logits = self.direct_model(
            input_ids,
            labels=labels,
            attention_mask=attention_mask.to(self.direct_model.device),
            decoder_input_ids=decoder_input_ids,#decoder_input_ids.to(self.direct_model.device),
            encoder_outputs=dm_encoder_outputs,
            decoder_attention_mask=decoder_attention_mask,
            past_key_values=None,
            use_cache=use_cache, # TODO: check
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        ).logits[:, -1, :].to(self.device)
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            model_inputs = self.direct_model.prepare_inputs_for_generation(input_ids, **model_kwargs)
            model_inputs["encoder_outputs"] = tuple(model_inputs["encoder_outputs"].last_hidden_states)[0]

            outputs = self.direct_model(
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
            ...
            input_ids = torch.cat([input_ids[beam_idx, :], beam_next_tokens.unsqueeze(-1)], dim=-1)
            if input_ids.shape[-1] == 2:
                dm_scores = beam_scores
            else:
                dm_scores = dm_scores[beam_idx] + direct_model_scores[beam_idx, beam_next_tokens]

            if input_ids.shape[-1] > 2:
                # prepare inputs for channel model
                cm_input_ids = []
                for label, lm_input in zip(input_ids, lm_input_ids):
                    cm_input = torch.cat([
                        lm_input[:(lm_input == 2).nonzero(as_tuple=True)[0]].cpu(),
                        torch.tensor(self.tokenizer.convert_tokens_to_ids(["<agent>"]) + self.tokenizer("".join(self.tokenizer.batch_decode(label.cpu(), skip_special_tokens=True)), add_special_tokens=False)["input_ids"] + self.tokenizer.convert_tokens_to_ids(["</s>"]))
                    ]).to(self.channel_model.device)
                ...
                beam_outputs["next_beam_scores"] = dm_scores + torch.mul(cm_scores, self.cm_scaling_factor) + torch.mul(lm_scores, self.lm_scaling_factor) #/ (input_ids.shape[-1] ** 1.0)
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if input_ids.shape[-1] > 2:
                ...
                beam_outputs["next_beam_scores"] = dm_scores + torch.mul(cm_scores, self.cm_scaling_factor) + torch.mul(lm_scores, self.lm_scaling_factor)
...
class OnlineNoisyChannelModelLiuEtAl(OnlineNoisyChannelModelForConditionalGeneration):

    def _get_noisy_channel_score(self, input_ids, next_tokens, model_kwargs):
        num_beams, batch_size = input_ids.shape[0], 1
        if input_ids.shape[-1] == 3:
            model_kwargs["lm_input_ids"] = torch.repeat_interleave(model_kwargs["lm_input_ids"], num_beams*self.k_2, dim=0)
            model_kwargs["encoder_outputs"].last_hidden_states[-1].last_hidden_state = model_kwargs["encoder_outputs"].last_hidden_states[-1].last_hidden_state.repeat_interleave(self.k_2, dim=0)
            model_kwargs["cm_labels"] = torch.repeat_interleave(model_kwargs["cm_labels"], num_beams*self.k_2, dim=0)
        lm_encoder_outputs = model_kwargs["encoder_outputs"].last_hidden_states[-1]
        lm_input_ids = model_kwargs["lm_input_ids"]
        cm_labels = model_kwargs["cm_labels"]
        ...
        input_ids = torch.repeat_interleave(input_ids, self.k_2, dim=0)
        input_ids = torch.cat([input_ids[:,1:], next_tokens.view(num_beams * self.k_2).unsqueeze(dim=-1)], dim=-1)
        # prepare input for channel model
        cm_input_ids = []
        ...
        lm_encoder_outputs.last_hidden_state = lm_encoder_outputs.last_hidden_state.to(self.language_model.device)
        lm_logits = self.language_model(input_ids=None, encoder_outputs=lm_encoder_outputs, labels=input_ids).logits
        lm_scores = -loss_fct(lm_logits.view(-1, self.config.vocab_size), input_ids.view(-1)).view(batch_size * (num_beams*self.k_2), input_ids.shape[-1]) 
        del lm_logits
        lm_encoder_outputs.last_hidden_state = lm_encoder_outputs.last_hidden_state.cpu()
        lm_scores = torch.sum(lm_scores, dim=-1)
        ...
        cm_logits = self.channel_model(cm_input_ids, labels=cm_labels_final, attention_mask=cm_attention_mask.to(self.channel_model.device)).logits
        cm_scores = -loss_fct(cm_logits.view(-1, self.config.vocab_size), cm_labels_final.view(-1)).view(batch_size * (num_beams *self.k_2), cm_labels_final.shape[-1]) 
        cm_scores = torch.sum(cm_scores, dim=-1)

        return torch.mul(lm_scores, self.lm_scaling_factor) + torch.mul(cm_scores, self.cm_scaling_factor)
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            model_inputs = self.direct_model.prepare_inputs_for_generation(input_ids, **model_kwargs)
            model_inputs["encoder_outputs"] = model_inputs["encoder_outputs"].last_hidden_states[0]
            ...
            if input_ids.shape[-1] > 2:
                ...
                next_token_scores = next_token_scores.view(batch_size, num_beams, vocab_size)
                next_token_scores, next_tokens = torch.topk(
                    next_token_scores, self.k_2, dim=-1, largest=True, sorted=True
                )
                next_token_scores = next_token_scores.view(-1, num_beams * self.k_2)
                next_tokens = next_tokens.view(-1, num_beams * self.k_2)
                next_indices = torch.arange(num_beams) \
                    .repeat_interleave(self.k_2) \
                    .repeat(batch_size) \
                    .view(batch_size, num_beams*self.k_2) \
                    .to(next_tokens.device)#
            else:
                next_token_scores = next_token_scores.view(batch_size, num_beams * vocab_size)
                next_token_scores, next_tokens = torch.topk(
                    next_token_scores, 2 * num_beams, dim=1, largest=True, sorted=True
            ...
            if input_ids.shape[-1] > 2:
                next_token_scores = dm_scores[next_indices] + direct_model_scores[next_indices, next_tokens]
                next_token_scores += self._get_noisy_channel_score(input_ids.clone(), next_tokens, model_kwargs)
                next_token_scores, indices = torch.topk(next_token_scores, 2 * num_beams, dim=-1)
                next_tokens = next_tokens.gather(-1, indices)
                next_indices = next_indices.gather(-1, indices)
